...
type DriverServiceClient interface {
	...
	StartJob(ctx context.Context, in *StartJobRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ProgressUpdate], error)
...
func (c *driverServiceClient) StartJob(ctx context.Context, in *StartJobRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ProgressUpdate], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &DriverService_ServiceDesc.Streams[0], DriverService_StartJob_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[StartJobRequest, ProgressUpdate]{ClientStream: stream}
...
type DriverServiceServer interface {
	// The method called by the Taskmaster to start a new job. The parameter contains the job specification and the list of actions to be executed.
	StartJob(*StartJobRequest, grpc.ServerStreamingServer[ProgressUpdate]) error
	// The method called by the Taskmaster to cancel the job. The parameter contains the job identifier.
...
func (UnimplementedDriverServiceServer) StartJob(*StartJobRequest, grpc.ServerStreamingServer[ProgressUpdate]) error {
...
func _DriverService_StartJob_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(DriverServiceServer).StartJob(m, &grpc.GenericServerStream[StartJobRequest, ProgressUpdate]{ServerStream: stream})
}
