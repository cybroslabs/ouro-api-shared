...
from sabledocs.proto_model import (
    MessageField,
    Message,
    EnumValue,
    Enum,
...
class ParseContext:
    ...
    def WithPath(self, path):
        return ParseContext(
            self.config, self.package, self.source_file_path, path, self.locations
        )
    ...
    def GetComments(self, path=""):
        ...
        if location is None:
            ...
            for ignore_line in self.config.ignore_comment_lines_containing:
                comments = "\n".join(
                    [c for c in comments.splitlines() if ignore_line not in c]
                )
            return comments
...
def parse_enum(
    enum: EnumDescriptorProto, ctx: ParseContext, parent_message, nested_type_chain: str
):
    ...
    for i, ev in enumerate(enum.value):
        e.values.append(
            parse_enum_value(ev, ctx.ExtendPath(COMMENT_ENUM_VALUE_INDEX, i))
        )
...
def parse_enums(
    enums: Sequence[EnumDescriptorProto],
    ctx: ParseContext,
    parent_message,
    nested_type_chain: str,
    config: SableConfig,
):
    for i, enum in enumerate(enums):
        ctx.package.enums.append(
            parse_enum(enum, ctx.ExtendPath(i), parent_message, nested_type_chain)
        )
...
def parse_field(
    ...
    mf.full_type = (
        ...
        if field.type_name != ""
        else to_type_name(field.type)
    )
    mf.default_value = field.default_value
    mf.type = (
        extract_type_name_from_full_name(field.type_name.strip("."))
        if field.type_name != ""
        else to_type_name(field.type)
    )
    mf.type_kind = (
        "MESSAGE"
        if field.type == FIELD_TYPE_MESSAGE
        else "ENUM" if field.type == FIELD_TYPE_ENUM else "UNKNOWN"
    )

    if mf.type.endswith("Entry"):
        entry_nested_type = next(
            filter(lambda m: m.full_name == mf.full_type, ctx.package.messages), None
        )
...
def parse_message(
    ...
    parse_messages(
        message.nested_type,
        ctx.ExtendPath(COMMENT_MESSAGE_MESSAGE_INDEX),
        m,
        new_nested_type_chain,
        config,
    )
    parse_enums(
        message.enum_type,
        ctx.ExtendPath(COMMENT_MESSAGE_ENUM_INDEX),
        m,
        new_nested_type_chain,
        config,
    )
    ...
    for i, mf in enumerate(message.field):
        m.fields.append(
            parse_field(mf, message, ctx.ExtendPath(COMMENT_MESSAGE_FIELD_INDEX, i))
        )
    ...
    m.oneof_field_groups = list(
        map(
            lambda n: OneOfFieldGroup(
                n, list(filter(lambda f: f.oneof_name == n, m.fields))
            ),
            oneof_names,
        )
    )
...
def parse_messages(
    messages: Sequence[DescriptorProto],
    ctx: ParseContext,
    parent_message,
    nested_type_chain: str,
    config: SableConfig,
    ...
    for i, message in enumerate(messages):
        ctx.package.messages.append(
            parse_message(
                message, ctx.ExtendPath(i), parent_message, nested_type_chain, config
            )
        )
...
def parse_service(service: ServiceDescriptorProto, ctx: ParseContext):
    ...
    for i, service_method in enumerate(service.method):
        s.methods.append(
            parse_service_method(
                service_method, ctx.ExtendPath(COMMENT_SERVICE_METHOD_INDEX, i)
            )
        )
...
def parse_services(
    services: Sequence[ServiceDescriptorProto], ctx: ParseContext, config: SableConfig
):
...
def add_package_references(
    ...
    for m in messages:
        ...
        for mf in m.fields:
            ...
            package = next(
                filter(lambda p: mf.full_type.startswith(p.name), packages), None
            )
            if package is not None:
                mf.package = package
                mf.is_package_hidden = any(
                    filter(lambda p: mf.full_type.startswith(p), hidden_packages)
                )
    ...
    for s in services:
        for sm in s.methods:
            requestPackage = next(
                filter(
                    lambda p: extract_package_name_from_full_name(sm.request.full_type)
                    == p.name,
                    packages,
                ),
                None,
            ...
            responsePackage = next(
                filter(
                    lambda p: extract_package_name_from_full_name(sm.response.full_type)
                    == p.name,
                    packages,
                ),
                None,
...
def parse_proto_descriptor(sable_config: SableConfig):
    ...
    with open(sable_config.input_descriptor_file, mode="rb") as proto_descriptor_file:
        ...
        for file in fds.file:
            ...
            package.description_html = markdown_to_html(
                package.description, sable_config
            )

            parse_enums(
                file.enum_type, ctx.WithPath(COMMENT_ENUM_INDEX), None, "", sable_config
            )
            parse_messages(
                file.message_type,
                ctx.WithPath(COMMENT_MESSAGE_INDEX),
                None,
                "",
                sable_config,
            )
            parse_services(
                file.service, ctx.WithPath(COMMENT_SERVICE_INDEX), sable_config
            )
